...
$partidasAbiertasQuery = mysqli_query($con, "SELECT COUNT(*) as abiertas 
                                             FROM partidas 
                                             WHERE estadoId != 4"); // Asume que '4' es el estado cerrado
...
try {
    ...
    $resultado = mysqli_query($con, "SELECT SUM(d.debe) as totalDebe, SUM(d.haber) as totalHaber
                                     FROM detalle d 
    ...
    $ganancias = $row['totalDebe'] - $row['totalHaber'];
    ...
    $query = "INSERT INTO partidas (codigoPartida, fechacontable, concepto, debe, haber, usuarioAgrega, fechaAgrega)
              VALUES ('CIERRE_ANUAL_$anioActual', NOW(), 'Cierre Anual $anioActual', $cargo, $abono, 'admin', NOW())";
    if (!mysqli_query($con, $query)) {
    ...
    // Inserción en partidaDetalle
    $query = "INSERT INTO partidaDetalle (partidaId, cuentaId, cargo, abono, concepto, usuarioAgrega, fechaAgrega)
              VALUES ($partidaId, $cuentaId, $cargo, $abono, 'Cierre Anual $anioActual', 'admin', NOW())";
    if (!mysqli_query($con, $query)) {
    ...
    // Vaciar solo los registros de partidaDetalle relacionados con las partidas del año que estás cerrando
    $query = "DELETE FROM partidaDetalle WHERE partidaId IN (SELECT partidaId FROM partidas WHERE fechacontable BETWEEN '$anioActual-01-01' AND '$anioActual-12-31')";
    if (!mysqli_query($con, $query)) {
    ...
    mysqli_commit($con);
    echo json_encode(['success' => true, 'message' => 'Cierre anual realizado exitosamente.']);
